...
LATEST_VERSION=$(gh release list --repo nwg-piotr/nwg-drawer --json name,tagName,isLatest --jq '.[] | select(.isLatest)|.tagName')
...
gh release download -R nwg-piotr/nwg-drawer --archive=tar.gz --output "${VERSION}.tar.gz"
...
printf "nwg-drawer template updated\n"
