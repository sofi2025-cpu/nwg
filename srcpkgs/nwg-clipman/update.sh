...
LATEST_VERSION=$(gh release list --repo nwg-piotr/nwg-clipman --json name,tagName,isLatest --jq '.[] | select(.isLatest)|.tagName')
...
gh release download -R nwg-piotr/nwg-clipman --archive=tar.gz --output "${VERSION}.tar.gz"
