...
LATEST_VERSION=$(gh release list --repo Letdown2491/nebula-gtk --json name,tagName,isLatest --jq '.[] | select(.isLatest)|.tagName')
...
gh release download -R Letdown2491/nebula-gtk --archive=tar.gz --output "${VERSION}.tar.gz"
...
printf "nebula-gtk template updated\n"
