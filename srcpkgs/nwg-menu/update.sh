...
LATEST_VERSION=$(gh release list --repo nwg-piotr/nwg-menu --json name,tagName,isLatest --jq '.[] | select(.isLatest)|.tagName')
...
gh release download -R nwg-piotr/nwg-menu --archive=tar.gz --output "${VERSION}.tar.gz"
...
printf "nwg-menutemplate updated\n"
