...
LATEST_VERSION=$(gh release list --repo nwg-piotr/nwg-displays --json name,tagName,isLatest --jq '.[] | select(.isLatest)|.tagName')
...
gh release download -R nwg-piotr/nwg-displays --archive=tar.gz --output "${VERSION}.tar.gz"
...
printf "nwg-displays template updated\n"
