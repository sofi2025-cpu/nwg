...
LATEST_VERSION=$(gh release list --repo anufrievroman/waypaper --json name,tagName,isLatest --jq '.[] | select(.isLatest)|.tagName')
...
gh release download -R anufrievroman/waypaper --archive=tar.gz --output "${VERSION}.tar.gz"
