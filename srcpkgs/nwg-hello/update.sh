...
LATEST_VERSION=$(gh release list --repo nwg-piotr/nwg-hello --json name,tagName,isLatest --jq '.[] | select(.isLatest)|.tagName')
...
gh release download -R nwg-piotr/nwg-hello --archive=tar.gz --output "${VERSION}.tar.gz"
...
printf "nwg-hello template updated\n"
