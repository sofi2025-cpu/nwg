...
LATEST_VERSION=$(gh release list --repo nwg-piotr/nwg-dock-hyprland --json name,tagName,isLatest --jq '.[] | select(.isLatest)|.tagName')
...
gh release download -R nwg-piotr/nwg-dock-hyprland --archive=tar.gz --output "${VERSION}.tar.gz"
...
printf "nwg-dock-hyprland template updated\n"
