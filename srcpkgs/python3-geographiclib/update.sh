...
LATEST_VERSION=$(gh release list --repo geographiclib/geographiclib-python --json name,tagName,isLatest --jq '.[] | select(.isLatest)|.tagName')
...
gh release download -R /python3-geographiclib --archive=tar.gz --output "${VERSION}.tar.gz"
