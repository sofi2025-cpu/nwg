...
LATEST_VERSION=$(gh release list --repo nwg-piotr/nwg-launchers --json name,tagName,isLatest --jq '.[] | select(.isLatest)|.tagName')
...
gh release download -R nwg-piotr/nwg-launchers --archive=tar.gz --output "${VERSION}.tar.gz"
...
printf "nwg-launchers template updated\n"
