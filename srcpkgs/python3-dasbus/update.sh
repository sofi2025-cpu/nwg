...
LATEST_VERSION=$(gh release list --repo dasbus-project/dasbus --json name,tagName,isLatest --jq '.[] | select(.isLatest)|.tagName')
...
gh release download -R dasbus-project/dasbus --archive=tar.gz --output "${VERSION}.tar.gz"
