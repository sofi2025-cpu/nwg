...
LATEST_VERSION=$(gh release list --repo nwg-piotr/nwg-icon-picker --json name,tagName,isLatest --jq '.[] | select(.isLatest)|.tagName')
...
gh release download -R nwg-piotr/nwg-icon-picker --archive=tar.gz --output "${VERSION}.tar.gz"
...
printf "nwg-icon-picker template updated\n"
