...
LATEST_VERSION=$(gh release list --repo imageio/imageio-ffmpeg --json name,tagName,isLatest --jq '.[] | select(.isLatest)|.tagName')
...
gh release download -R imageio/imageio-ffmpeg --archive=tar.gz --output "${VERSION}.tar.gz"
...
printf "python3-imageio-ffmpeg updated\n"
