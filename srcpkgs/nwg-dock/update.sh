...
LATEST_VERSION=$(gh release list --repo nwg-piotr/nwg-dock --json name,tagName,isLatest --jq '.[] | select(.isLatest)|.tagName')
...
gh release download -R nwg-piotr/nwg-dock --archive=tar.gz --output "${VERSION}.tar.gz"
...
printf "nwg-dock template updated\n"
