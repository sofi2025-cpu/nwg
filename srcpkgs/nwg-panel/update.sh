...
LATEST_VERSION=$(gh release list --repo nwg-piotr/nwg-panel --json name,tagName,isLatest --jq '.[] | select(.isLatest)|.tagName')
...
gh release download -R nwg-piotr/nwg-panel --archive=tar.gz --output "${VERSION}.tar.gz"
