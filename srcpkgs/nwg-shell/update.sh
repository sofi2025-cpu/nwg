...
LATEST_VERSION=$(gh release list --repo nwg-piotr/nwg-shell --json name,tagName,isLatest --jq '.[] | select(.isLatest)|.tagName')
...
gh release download -R nwg-piotr/nwg-shell --archive=tar.gz --output "${VERSION}.tar.gz"
