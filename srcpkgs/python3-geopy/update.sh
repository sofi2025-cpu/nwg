...
LATEST_VERSION=$(gh release list --repo geopy/geopy --json name,tagName,isLatest --jq '.[] | select(.isLatest)|.tagName')
...
gh release download -R geopy/geopy --archive=tar.gz --output "${VERSION}.tar.gz"
