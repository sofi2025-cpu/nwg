...
LATEST_VERSION=$(gh release list --repo nwg-piotr/nwg-shell-config --json name,tagName,isLatest --jq '.[] | select(.isLatest)|.tagName')
...
gh release download -R nwg-piotr/nwg-shell-config --archive=tar.gz --output "${VERSION}.tar.gz"
...
printf "nwg-shell-config updated\n"
