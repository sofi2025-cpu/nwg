...
LATEST_VERSION=$(gh release list --repo nwg-piotr/nwg-bar --json name,tagName,isLatest --jq '.[] | select(.isLatest)|.tagName')
...
gh release download -R nwg-piotr/nwg-bar --archive=tar.gz --output "${VERSION}.tar.gz"
...
printf "nwg-bar template updated\n"
